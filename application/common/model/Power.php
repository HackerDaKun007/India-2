...
class Power extends Common
{
    ...
    protected $allow = ['username', 'whether', 'grade', 'level', 'controller', 'method', 'sort', 'url'];
    ...
    public function add($data) {
        $msg = '添加失败';
        $code = 0;
        if(self::allowField($this->allow)->isUpdate(false)->save($data)) {
            $msg = '添加成功';
            $code = 1;
           $this->cacheUpload($data['grade']);
        };
        return $this->dataJson($code, $msg, '', '', true);
    }
    ...
    public function edit($data) {
        $msg = '修改失败';
        $code = 0;
        if(self::allowField($this->allow)->isUpdate(true)->save($data, ['power_id'=>$data['power_id']])) {
            $msg = '修改成功';
            $code = 1;
            $this->cacheUpload($data['grade']);
        };
     ...
     */
    public function cacheUpload($grade) {
        $model1 = '';
        $model2 = '';
        $model3 = '';
        if($grade == 1 || $grade == 2  || $grade == 3) {
            $model = self::where([['grade', 'eq', $grade]])->order('sort desc')->select()->toArray();
            $this->cache::set($this->path['modelPowerW'. $grade], $model);
            if($grade == 1) {
                $model1 = $model;
            }else if($grade == 2) {
                $model2 = $model;
            ...
            $this->cache::set($this->path['modelPowerW1'], $model1);
            $this->cache::set($this->path['modelPowerW2'], $model2);
            $this->cache::set($this->path['modelPowerW3'], $model3);
        }
        $this->cacheShow($model1, $model2, $model3);
...
//        cache(self::$path['modelPower'], array_column($model, 'url'));
    ...
    public function yzCache($val) {
        $data = $this->cache::get($this->path[$val]);
        if(empty($data)) {
            $this->cacheUpload(false);
            $data = $this->cache::get($this->path[$val]);
        }
     ...
     * 更新modelPowerShow缓存
     */
    protected function cacheShow($data1='', $data2='', $data3='') {
        if(empty($data1)) {
            $data1 = $this->cache::get($this->path['modelPowerW1']);
        }if(empty($data2)) {
            $data2 = $this->cache::get($this->path['modelPowerW2']);
        }if(empty($data3)) {
            $data3 = $this->cache::get($this->path['modelPowerW3']);
        }
        $data = [];
        if(!empty($data1)) {
            ...
            $this->cache::set($this->path['modelPowerShow'], $data);
        }
