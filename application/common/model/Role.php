...
class Role extends Common {
    ...
    public function add($data, $bool) {
        ...
        if(self::where($where)->count()) {
            ...
            if(self::allowField($this->allow)->isUpdate($bool)->save($data, $id)) {
                if($bool == true){
                    $this->cacheShow($data['power_id'], $data['role_id'], $data['admin']);
                }else {
                    $this->cacheShow($data['power_id'], $this->id, $data['admin']);
                }
                $msg = '操作成功';
                $code = 1;
            };
    ...
    protected  function cacheShow($power_id, $id, $admin) {
        $Power = new \app\common\model\Power($this->path,$this->cache);
        $s = $Power->where([['power_id', 'in', $power_id]])->order('sort desc')->select()->toArray();
        ...
        $this->cache::set($this->path['modelRoleShow'] .'_'. $id, $data);
        $this->cache::set($this->path['modelRoleSelect'] .'_'. $id, $select);
    }
    ...
    public function del($id) {
        ...
        if(self::where('role_id','=', $id)->delete()) {
            $this->cache::rm($this->path['modelRoleShow'] .'_'. $id);
            $this->cache::rm($this->path['modelRoleSelect'] .'_'. $id);
            $msg = '删除成功';
            $code = 1;
        };
        return self::dataJson($code, $msg, '', '', true);
    ...
    public function modelRoleSelect($id) {
        $data = $this->cache::get($this->path['modelRoleSelect'] .'_'. $id);  //获取角色权限路径
        if(!$data) {
            $find = self::where('role_id', '=', $id)->find();
            if($find) {
                $this->cacheShow($find['power_id'], $id, $find['admin']);
                $data = $this->cache::get($this->path['modelRoleSelect'] .'_'. $id);  //获取角色权限路径
            }
        }
        return $data;
